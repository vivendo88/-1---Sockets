...
def WorkSelectRoom(device, msg, controlQueue):
	...
	if roomItem != None:
		...
		if device.typeCode == COD_LAMPADA or device.typeCode == COD_AR_CONDICIONADO:
			device.lampQueue = queue.Queue()
			print('Enviando mensagem de novo atuador para a fila do controle')
...
def WorkSensor(device, msg, controlQueue):
	...
	####################################################
	# Se um sensor de presença ou termômetro foi acionado, informar ao controle
	if device.typeCode == COD_SENSOR_PRESENCA or device.typeCode == COD_TERMOMETRO:
...
def DeviceThread(connection, clientIP, controlQueue):
	...
	if device.ID != None:
		if device.typeCode == COD_LAMPADA or device.typeCode == COD_AR_CONDICIONADO:
			# Remove o atuador da lista do ambiente
